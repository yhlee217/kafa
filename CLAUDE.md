...
3. ⏸️ 간이과세자 자동 불공제 식별자 — 미해결(`kafa-learn` 으로 '구분' 컬럼 확인 중)
4. ⏸️ 카면(58) 실제 처리·식당 의제 검증 — 의제 율은 매출 규모별이라 신고 단계 이관(설계 확인됨)
5. ⏸️ 계정과목 전체 코드표(위하고 '기초데이터 › 계정과목(변경)'에서 수령 예정)
6. ⏸️ 거래구분 허용값 / .xlsx 업로드 허용 / 대변거래처 양식 위치
