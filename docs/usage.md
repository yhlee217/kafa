...
**AI 계정 추정을 켜려면** 서버 환경에 API 키를 넣습니다(미추천 행의 차변계정을 Claude 가
업종·품명만 보고 추정 + 근거 생성). 키가 없으면 자동으로 로컬 시드 추천으로 동작합니다.
```json
{
  "mcpServers": {
    "kafa": { "command": "kafa-mcp", "env": { "ANTHROPIC_API_KEY": "sk-ant-..." } }
  }
}
```
거래처 실명·사업자번호는 **AI 에 전달되지 않습니다**(업태/종목/품명/유형만 사용). CLI 에서도
`ANTHROPIC_API_KEY` 환경변수가 있으면 동일하게 AI 추정이 동작합니다.
...
## 보장
- **오차 없음/고정 템플릿**: 위하고가 채운 행의 매핑·코드 변환은 결정론적 룰 엔진이 수행,
  출력은 고정 컬럼 스키마(.xls)로만. AI 추정 결과는 **결정 캐시**로 같은 입력 → 같은 결과.
- **AI 추정**: 미추천(차변계정 미정) 행만 Claude 가 업종/품명/유형으로 추정하고 근거를 생성.
  거래처 실명·사업자번호는 AI 에 전달하지 않음. 결과 .xls/CSV 에 계정코드와 근거가 함께 남음.
- **보안**: 원천 데이터는 로컬 코드만 처리. AI 에게는 마스킹 요약 + 비-PII 특징만 전달.
