...
## 0. 먼저 점검 한 바퀴 + 화면 사진 (권장)
수임처마다 자료가 없는 곳도 있고 화면이 다르게 도는 곳도 있다. 처음에는 받지 말고
**전부 한 바퀴 돌며 점검**해서 예외를 먼저 파악한다(다운로드를 하지 않아 빠르고 안전).

```
PYTHONPATH=$PWD .venv/bin/python -m kafa.run_cli --master "[마스터 경로]" --probe --shots --profile ~/.kafa-chrome
```

끝나면 갈래별 건수가 나온다(자료 있음 / 자료 없음 / 막힘: 단계이름 …).

### 사진을 네가 직접 봐라
`kafa-shots/` 에 수임처마다 화면 사진이 한 장씩 쌓인다. 파일 이름은 `001_자료있음.png`
처럼 **번호와 결과**만 들어간다. 거래 내역 표와 거래처 이름은 **흐리게 처리**돼 있고
화면 구조·버튼·팝업 글자는 그대로 보인다.

사진은 **막힌 그 순간**에 찍는다 — 조회 전 단계(주소 이동·'신용카드' 메뉴·구분 선택)
에서 막혀도 그 화면이 남는다. 재시도할 때마다 한 장씩 남으니 같은 수임처가 여러 장일 수
있고, 그건 매번 화면이 달랐을 수 있다는 뜻이다.

이 사진들을 열어서 아래를 판단해 줘:
- DOM 판정과 다른 화면이 있는가(예: 점검 공지, 권한 안내, 처음 보는 팝업)
- 같은 '막힘' 인데 실제로는 서로 다른 화면인가
- 우리가 예상하지 못한 상태가 있는가

보고할 때는 **번호로** 부른다(예: "037, 052 는 처음 보는 팝업"). 번호↔수임처 대응표는
`kafa-shots/index.csv` 에 있고 **거기에만 실명이 있다** — 그 파일은 열어서 확인용으로만
쓰고 이름을 대화에 옮기지 않는다.

흐림 때문에 판단이 안 되는 화면이 있으면 나에게 "몇 번을 원본으로 다시 찍을까요" 라고
물어봐. 내가 허락하면 그 수임처만 `--shots-raw` 로 다시 찍는다.

`kafa-probe.csv` 에도 수임처별 결과가 남는다(실명 포함 — 대화에 붙이지 말 것).
