...
- 2026-08-31: [자율 사이클] `io_wehago/account_sheet.py` 테스트 커버리지 보강. 실제 양식
  (2026-08-06 확보)의 핵심 구조인 **분류별 (코드, 계정과목) 쌍 가로 반복 + 원가구분 접두
  마커((제)/(도)/(분)/(판))** 경로가 그동안 단위테스트로 전혀 검증되지 않았음을 커버리지
  측정(`coverage`)으로 발견(75%, `_parse_pairs` 전체 미실행). 합성 시트로 ① 쌍 구조 파싱+마커
  적용, ② 쌍 구조가 단순 컬럼탐지로 오인되지 않음, ③ 쌍 헤더는 찾았으나 데이터가 전부
  공란이면 단순 구조 폴백도 실패해 예외, ④ 시트 이름이 안 맞으면 첫 시트로 폴백,
  ⑤ `build_mapping`이 쌍 구조 시트도 정상 머지 테스트 추가(98%). 코드 변경 없음(테스트만),
  단위테스트 +8(총 321).
