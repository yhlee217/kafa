...
- 2026-06-29: [리포트 개선 + 테스트 커버리지] ① `render_report` 체크포인트에 **의제매입 대상 후보 N건** 항목 추가 — 헤더에는 수치가 있었으나 담당자 액션이 필요한 체크포인트에는 없었음. ② `build_review(known_biznos=...)` 경로('마스터 미등록 신규 거래처' 플래그)가 구현 있으나 호출처·테스트 전무 → 테스트 추가. 합성 데이터만 사용. 테스트 179→181.
