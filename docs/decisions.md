...
## 변경 이력
- 2026-06-14: [자율 사이클] 음수(환불)+미추천 행의 `finalize_reversal` 통합 흐름 테스트 추가.
  `classify_rows` 경로에서 추천 성공/실패 시 swap 확정 여부를 검증. 단위테스트 123케이스.
...
- 2026-08-10: [자율 사이클: 테스트 보강] `render_board_html`(진행 현황 보드) 의 HTML 이스케이프
  회귀 테스트 추가. `client_id`는 담당자가 정한 로컬 폴더명이라 `&`/`<`/`>` 등을 포함할 수 있는데
  (예: "A&B상사"), 기존엔 `_esc` 적용 여부를 검증하는 테스트가 없었음 — 이스케이프 로직이
  깨져도 조용히 통과했을 것. 특수문자 섞인 client_id로 렌더링해 이스케이프됨을 확인.
  코드 변경 없음(기존 `_esc` 정상 동작 확인). 단위테스트 +1(총 194).
